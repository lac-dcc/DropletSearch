...
def evaluate_performance(lib, data_shape):
    ...
    # evaluate
    print(module.benchmark(dev, number=10, repeat=5))


def tune_and_evaluate(tuning_opt, log_file, model, arch, tuner):
    ...
    tuning_option = {
        ...
        "measure_option": autotvm.measure_option(
            ...
            runner=autotvm.LocalRunner(
                number=2, repeat=5, min_repeat_ms=0, enable_cpu_cache_flush=True
            ),
        ),
    }

    tune_and_evaluate(tuning_option, log_file, model, arch, tuner)
